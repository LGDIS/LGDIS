...
class SetupXML
	require "date"
		def self.arrange_and_put(options = nil)
			xml = SetupXML.get('/opt/redmine/plugins/lgdis/lib/lgdis/ext_out/rss1_0.tmpl', :mime_type => "application/rss+xml")
...
#             },
        ...
        outfile = "/opt/redmine/public/atom/#{time}-Sent.log"
			else
            ...
            issue.issues_additional_data.each {|i|
              str_xy3   += i.latitude.to_s + " " + i.longitude.to_s + " "
				...
				xml.items = [
				     ...
				     :uuid      => `uuidgen`.chomp,
						 :description => "1234567890"*14,
						 :date      => time,
						 :point     => str_xy1.to_s ,
						 :line      => str_xy2.to_s  , 
						 :polygon   => str_xy3.to_s #latitude1 + " " + longitude1 + " " + latitude2 + " " + longitude2 + " 45.256 -110.45"
						}
				]
