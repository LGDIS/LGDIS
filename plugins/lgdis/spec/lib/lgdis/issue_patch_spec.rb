...
describe Lgdis::IssuePatch do
  ...
  describe "#custom_field_value_by_id " do
    before do
      @issue.save
      @cv = CustomValue.where(:customized_id =>@issue.id, :custom_field_id=>5)[0]
      @cv.value ="test_value"
      @cv.save
    end
    it "get custom_value" do
      @issue.custom_field_value_by_id(@cv.custom_field_id).should == @cv.value
    end
  end
  ...
  describe "#create_twitter_msg " do
    before do
      @issue.save
      @cv = CustomValue.where(:customized_id =>@issue.id, :custom_field_id=>4)[0] #  DST_LIST['smtp']['target_num']=4
      @cv.value ="twitter_msg"
      @cv.save
    end
    it "get summary" do
      @issue.create_twitter_msg.should == @cv.value
    end
  end
  ...
  describe "#create_smtp_msg " do
    ...
    it "get summary" do
      ...
      summary["title"].should == @issue.subject
