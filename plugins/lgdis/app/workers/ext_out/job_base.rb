  ...
  class JobBase
    ...
    def self.register_edition(content, delivery_history)
      ...
      if edition_mng.blank?
        ...
        type_update = "3" if tracker_id == 1 && EvacuationAdvisory.mode_in(issue.project).where("current_sort_criteria != ?", "1").exists? == false
        edition_mng.update_attributes(status:      type_update == "3" ? type_update : DST_LIST['type_update'].invert[xml_body.elements["//edxlde:EDXLDistribution/edxlde:distributionType"].text],
                                      issue_id:    issue.id,
                                      uuid:        type_update == "3" ? " " : xml_body.elements["//pcx_ib:Head/commons:documentID"].text,
                                      edition_num: type_update == "3" ? 0 : xml_body.elements["//pcx_ib:Head/commons:documentRevision"].text.to_i)
      end
    end
